...
def create_visual_wakeword_annotations(annotations_file,
    ...
    with tf.io.gfile.GFile(annotations_file, 'r') as fid:
        ...
        for idx, image in enumerate(images):
            if idx % 100 == 0:
                tf.compat.v1.logging.info(
                    'On image %d of %d', idx, len(images))
            ...
            annotations_index_filtered[image['id']].append(
                annotations_list_filtered)
        # Output Visual WakeWords annotations and labels
        labels_to_class_names = {0: 'background',
                                 1: foreground_class_of_interest}
        with open(visualwakewords_labels_filename, 'w') as fp:
            for label in labels_to_class_names:
                fp.write(str(label) + ':' +
                         str(labels_to_class_names[label]) + '\n')
